...
In this image, we can see the interaction between a Publisher and a Subscriber program using a message broker, which is RabbitMQ. When both the Publisher and Subscriber are executed (cargo run), the Publisher sends user data as messages to the message broker, and the Subscriber listens for these messages and receives them. The terminal on the right shows that the Publisher executed successfully, while the terminal on the left confirms that the Subscriber received the UserCreatedEventMessage multiple times, each containing user data such as ID and username. This demonstrates a successful message-passing mechanism: the Publisher sends data, and the Subscriber captures and prints each message upon arrival.</br>

<img width="1280" alt="run_publisher_multiple_rime" src="https://github.com/user-attachments/assets/e97d354e-7026-467b-abc3-b1d9d2752157" />

After running the Publisher multiple times, a visible spike appeared on the message rate chart in the RabbitMQ Management UI (as shown in the screenshot). This spike reflects the moment when the Publisher sent messages to the message broker. Each execution of the Publisher sends a UserCreatedEventMessage into the queue, which increases the message throughput rate temporarily. The chart visualizes this activity, confirming that messages are being successfully published and received by the system. This behavior verifies that our Publisher is working correctly and RabbitMQ is capturing and processing the events as expected.